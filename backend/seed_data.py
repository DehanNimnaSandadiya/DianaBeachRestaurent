...
# Compact 10 drinks dataset used for recommendations too.
FINAL_DRINKS = [
    "Mojito Classic",
    "Margarita",
    "Cosmopolitan",
...
# Ensure every nationality has reviews for the compact drinks set,
# so recommendations can rank them with real rating data.
for _nat, _dishes in list(NATIONALITY_DISHES.items()):
    combined = list(_dishes)
    for d in FINAL_DRINKS:
        if d not in combined:
            combined.append(d)
    NATIONALITY_DISHES[_nat] = combined
